...
async function findActiveReservationForUser(
  ...
  for (const reservation of reservations) {
    ...
    if (reservation.startTime <= referenceTimestamp && referenceTimestamp < endTime) {
      return reservation;
    }
  }
...
function utcDayBounds(referenceTimestamp: number): { dayEnd: number; dayStart: number } {
...
function getNextStatus(current: "pending" | "preparing" | "ready" | "completed"): "preparing" | "ready" | "completed" | null {
  if (current === "pending") {
    return "preparing";
  }
...
export const listActive = query({
  ...
  handler: async (ctx) => {
    ...
    const enriched = await Promise.all(combined.map((order) => toOrderWithTable(ctx, order)));
...
export const listCompletedToday = query({
  ...
  handler: async (ctx, args) => {
    ...
    const todays = completed.filter((order) => order.createdAt >= dayStart && order.createdAt < dayEnd);
    const enriched = await Promise.all(todays.map((order) => toOrderWithTable(ctx, order)));
...
export const listByUser = query({
  ...
  handler: async (ctx) => {
    ...
    const sorted = [...orders].sort((left, right) => right._creationTime - left._creationTime);
...
export const listActiveForUserAtTable = query({
  ...
  returns: v.array(orderWithTableValidator),
  handler: async (ctx, { referenceTimestamp: _referenceTimestamp, tableId }) => {
    const user = await requireAuth(ctx);
    const orders = await ctx.db
      .query("orders")
      .withIndex("by_userId", (query) => query.eq("userId", user._id))
    ...
    const active = orders.filter(
      (order) =>
        order.tableId === tableId && (order.status === "pending" || order.status === "preparing" || order.status === "ready"),
    );

    const enriched = await Promise.all(active.map((order) => toOrderWithTable(ctx, order)));
    return enriched.sort((left, right) => right.createdAt - left.createdAt);
  },
});
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (order.userId !== user._id && user.role !== "staff" && user.role !== "admin") {
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const activeReservation = await findActiveReservationForUser(ctx, user._id, args.tableId, args.referenceTimestamp);

    const orderType = activeReservation ? ("reserved" as const) : ("walkin" as const);
