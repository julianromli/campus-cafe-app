		...
		const sections = await Promise.all(
			sortedCategories.map(async (category) => {
				const items = await ctx.db
					.query("menuItems")
					.withIndex("by_categoryId", (query) =>
						query.eq("categoryId", category._id),
					)
					.collect();
...
export const listItemsByCategory = query({
	...
	handler: async (ctx, args) => {
		const items = await ctx.db
			.query("menuItems")
			.withIndex("by_categoryId", (query) =>
				query.eq("categoryId", args.categoryId),
			)
			.collect();
...
export const listAllItems = query({
	...
	handler: async (ctx) => {
		...
		return enriched.sort((left, right) => {
			const leftOrder =
				categories.find((category) => category._id === left.categoryId)
					?.displayOrder ?? 0;
			const rightOrder =
				categories.find((category) => category._id === right.categoryId)
					?.displayOrder ?? 0;
...
export const createCategory = mutation({
	...
	handler: async (ctx, args) => {
		...
		const nextOrder =
			existing.length === 0
				? 0
				: Math.max(...existing.map((category) => category.displayOrder)) + 1;
...
export const bulkToggleCategory = mutation({
	...
	handler: async (ctx, args) => {
		...
		await Promise.all(
			items.map((item) =>
				ctx.db.patch(item._id, { available: args.available }),
			),
		);
