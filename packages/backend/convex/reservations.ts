...
const durationHoursValidator = v.union(v.literal(1), v.literal(2), v.literal(3));
...
function getReservationEndTime(startTime: number, durationHours: 1 | 2 | 3): number {
...
function toReservationBoardItem(table: AppTable, reservation: AppReservation, user: AppUser | null) {
  return {
    ...reservation,
    customer: {
      email: user?.email ?? "",
...
async function getTableOrThrow(ctx: DbCtx, tableId: Id<"tables">): Promise<AppTable> {
...
async function ensureAvailability(
  ...
  const targetEndTime = getReservationEndTime(args.startTime, args.durationHours);
  const reservations = await listConfirmedReservationsForTable(ctx, args.tableId);

  for (const reservation of reservations) {
    if (args.excludeReservationId && reservation._id === args.excludeReservationId) {
      continue;
    }

    const existingEndTime = getReservationEndTime(reservation.startTime, reservation.durationHours);
    if (timeRangesOverlap(args.startTime, targetEndTime, reservation.startTime, existingEndTime)) {
      return false;
    }
  }
...
export const listAll = query({
  ...
  handler: async (ctx, args) => {
    ...
    const dateRange = args.date !== undefined ? parseDateFilter(args.date) : null;
    const filteredReservations =
      dateRange
        ? reservations.filter((reservation) => {
            return reservation.startTime >= dateRange.start && reservation.startTime < dateRange.end;
          })
        : reservations;
...
export const listForBoard = query({
  ...
  handler: async (ctx, args) => {
    ...
    const activeRangeEnd = args.date !== undefined ? parseDateFilter(args.date).end : null;
...
export const checkActiveForTable = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const reservation of reservations) {
      if (reservation.status !== "confirmed" || reservation.tableId !== args.tableId) {
        continue;
      }

      const endTime = getReservationEndTime(reservation.startTime, reservation.durationHours);
      if (reservation.startTime <= args.referenceTimestamp && args.referenceTimestamp < endTime) {
        const table = await getTableOrThrow(ctx, reservation.tableId);
        return toReservationWithTable(table, reservation);
      }
    }
...
export const cancel = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const updatedReservation = await getReservationOrThrow(ctx, reservation._id);
...
export const getPendingForPayment = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    if (!reservation || reservation.userId !== args.userId || reservation.status !== "pending") {
      return null;
    }
