...
const MAYAR_CREATE_PAYMENT_URL = process.env.MAYAR_PAYMENT_CREATE_URL ?? "https://api.mayar.id/hl/v1/payment/create";

function getReservationPricePerHour(): number {
  const amount = Number(process.env.RESERVATION_PRICE_PER_HOUR ?? "0");
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new Error("RESERVATION_PRICE_PER_HOUR must be configured before creating payments");
  }
...
function parseMayarCreateResponse(response: unknown): { transactionId: string; url: string } {
...
function parseWebhookPayload(body: unknown): {
  ...
  const transactionIdCandidates = [
    typeof outerPayload.paymentLinkTransactionId === "string" ? outerPayload.paymentLinkTransactionId : null,
    innerData && typeof innerData.transactionId === "string" ? innerData.transactionId : null,
    innerData && typeof innerData.id === "string" ? innerData.id : null,
  ];

  const transactionId = transactionIdCandidates.find((candidate) => candidate !== null) ?? undefined;
  const event =
  ...
  const status =
    typeof outerPayload.status === "string"
      ...
      : innerData && typeof innerData.status === "string"
        ? innerData.status
        : undefined;
  const amount = innerData && typeof innerData.amount === "number" ? innerData.amount : undefined;
...
export const createReservationPaymentLink = action({
  ...
  handler: async (ctx, args) => {
    ...
    const reservation = await ctx.runQuery(internal.reservations.getPendingForPayment, {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Mayar payment creation failed: ${responseText || response.statusText}`);
    }
...
export const mayarWebhook = httpAction(async (ctx, request) => {
  ...
  if (!payload.transactionId || payload.event !== "payment.received" || payload.status !== "SUCCESS") {
