...
export default function AdminTablesPage() {
  ...
  const handleMove = async (table: FloorPlanTable, nextPosition: { positionX: number; positionY: number }) => {
    try {
      await updateTable({
        id: table._id,
      ...
      toast.error(error instanceof Error ? error.message : "Failed to move table");
    }
  };
      ...
      <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Manage labels, capacity, and activation status before placing tables on the floor canvas.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="rounded-md border border-border p-3">
                <div className="text-lg font-semibold">{summary.inactive}</div>
            ...
            <div className="flex flex-col gap-3">
              {tables === undefined ? (
                ...
                tables.map((table) => (
                  <div key={table._id} className="flex flex-col gap-3 rounded-md border border-border p-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`rounded-full border px-2 py-1 text-[11px] ${getStatusClasses(table.status)}`}>
                        {getStatusLabel(table.status)}
                      </span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <Button size="sm" variant="outline" onClick={() => openEditPanel(table)}>
                        Edit
                      </Button>
                      <Button
                        size="sm"
                        ...
                        onClick={async () => {
                          try {
                            await setTableStatus({
                              id: table._id,
                              status: table.status === "inactive" ? "available" : "inactive",
                            });
                      ...
                      >
                        {table.status === "inactive" ? "Activate" : "Deactivate"}
                      </Button>
                      <Button
                        size="sm"
                        variant="destructive"
                        onClick={async () => {
                          if (!window.confirm(`Delete ${table.label}? This only works when there are no confirmed reservations.`)) {
                            return;
                          }
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Drag tables to update their coordinates. The same geometry contract is reused by the customer reserve view.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            {tables === undefined ? (
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button disabled={submitting} onClick={handleSubmit}>
              {submitting ? "Saving..." : panelMode === "create" ? "Create table" : "Save changes"}
            </Button>
          </div>
        }
        open={panelOpen}
        title={panelMode === "create" ? "Add table" : `Edit ${selectedTable?.label ?? "table"}`}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input
              ...
              onChange={(event) => setFormState((current) => ({ ...current, zone: event.target.value }))}
            />
          </div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                onChange={(event) => setFormState((current) => ({ ...current, positionX: event.target.value }))}
              />
            </div>
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                onChange={(event) => setFormState((current) => ({ ...current, positionY: event.target.value }))}
              />
            </div>
          </div>
        </div>
