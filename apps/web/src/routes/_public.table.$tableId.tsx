...
export default function TableLandingPage() {
  ...
  const table = useQuery(api.tables.getById, tableId ? { id: tableId } : "skip");
  ...
  if (!tableIdParam) {
    return <p className="text-sm text-muted-foreground">Meja tidak valid.</p>;
  }
  ...
  if (table === null || table.status === "inactive") {
    return (
      <div className="rounded-xl border border-border bg-card p-6 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Periksa kembali QR code atau hubungi staf.</p>
      </div>
    );
  }
    ...
    <CartProvider tableId={table._id}>
      <div className="flex max-w-4xl flex-col gap-4 pb-28">
        ...
        {user && activeReservation ? (
          <div className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-800 dark:text-emerald-200">
            Reservasi aktif di meja ini — pesanan akan tertaut ke booking kamu.
          </div>
        ) : null}
        {user && activeOrders && activeOrders.length > 0 ? <OrderStatusInline orders={activeOrders} /> : null}
        <MenuView onRequireAuth={() => setSignInOpen(true)} />
        <CartDrawer user={user} onRequireAuth={() => setSignInOpen(true)} />
        <SignInPromptSheet open={signInOpen} redirectPath={redirectPath} onOpenChange={setSignInOpen} />
      </div>
    </CartProvider>
  );
}
