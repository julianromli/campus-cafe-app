...
export default function ReservePage() {
  ...
  const initialDate = isValidDateInput(requestedDate) ? requestedDate : undefined;
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_280px]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Pick an available table, choose your schedule, then continue to Mayar to complete the booking payment.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            {tables === undefined ? (
              <p className="text-sm text-muted-foreground">Loading table availability...</p>
            ) : (
              <FloorPlan
        ...
        <Card size="sm">
          <CardHeader>
            ...
            <CardDescription>Availability updates reactively as table status changes.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-3 text-sm text-muted-foreground">
            ...
            <div className="rounded-md border border-border bg-muted/30 p-3 text-xs leading-6">
              Reservations stay pending until the payment webhook confirms them and the table status changes to booked.
            </div>
          </CardContent>
        </Card>
      </div>
