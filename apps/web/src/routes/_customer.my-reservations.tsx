...
export default function MyReservationsPage() {
  ...
  useEffect(() => {
    ...
    toast.success("Reservation created. We will update the status as soon as payment is confirmed.");
  ...
  const visibleReservations = activeTab === "upcoming" ? groupedReservations.upcoming : groupedReservations.past;
  const reservationPendingCancellation =
    confirmingReservationId !== null
      ? reservations?.find((reservation) => reservation._id === confirmingReservationId) ?? null
      : null;
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Track pending payments, confirmed bookings, and your upcoming table schedule in one place.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          {reservations === undefined ? (
            ...
            <div className="rounded-md border border-border p-4 text-sm text-muted-foreground">
              No reservations yet. Start from the reserve page to book your first table.
            </div>
          ) : (
      ...
      {visibleReservations.length > 0 ? (
        <section className="grid gap-4">
          <h2 className="text-lg font-semibold">{activeTab === "upcoming" ? "Upcoming" : "Past"}</h2>
          {visibleReservations.map((reservation) => (
            <Card key={reservation._id}>
              <CardHeader>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <CardDescription>
                      {reservation.table.zone} • {reservation.guestCount} guests • {reservation.durationHours} hour(s)
                    </CardDescription>
                  </div>
              ...
              <CardContent className="flex flex-col gap-2 text-sm text-muted-foreground">
                ...
                <div>Confirmation code: {reservation.status === "confirmed" ? reservation.confirmationCode : "Pending payment"}</div>
                {reservation.eventId ? <div>Linked event reservation</div> : null}
      ...
      <div>
        <Button variant="outline" onClick={() => window.location.assign("/reserve")}>
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            if (message === "Cancellation window passed") {
              toast.error("Pembatalan tidak dapat dilakukan kurang dari 2 jam sebelum jadwal.");
            } else {
              toast.error(message);
            }
          } finally {
