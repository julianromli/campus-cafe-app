...
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@campus-cafe/ui/components/table";
...
function roleBadgeVariant(role: "admin" | "customer" | "staff"): "default" | "secondary" {
...
export default function AdminStaffPage() {
  ...
  const findResult = useQuery(
    ...
    submittedEmail !== null && submittedEmail.length > 0 ? { email: submittedEmail } : "skip",
  );

  const [revokeUserId, setRevokeUserId] = useState<string | null>(null);
  const [pendingRevoke, setPendingRevoke] = useState(false);
  const [roleChangePendingId, setRoleChangePendingId] = useState<string | null>(null);

  const revokeTarget = revokeUserId ? staffList?.find((row) => row._id === revokeUserId) : null;

  return (
    <div className="grid gap-8">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Staff</h1>
        <p className="text-sm text-muted-foreground">Kelola akses staff dan admin, atau promosi pengguna yang sudah terdaftar.</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-base">Promosi berdasarkan email</CardTitle>
          <CardDescription>Cari pengguna yang sudah punya akun, lalu naikkan ke Staff atau Admin.</CardDescription>
        </CardHeader>
          ...
          <CardContent className="border-t border-border pt-4">
            {findResult === undefined ? (
              ...
              <p className="text-sm text-muted-foreground">
                Pengguna dengan email ini tidak ditemukan. Minta mereka untuk mendaftar dulu.
              </p>
            ) : findResult.role === "staff" || findResult.role === "admin" ? (
              <div className="flex flex-wrap items-center gap-3">
                <div>
                  <p className="font-medium text-foreground">{findResult.name}</p>
                  <p className="text-sm text-muted-foreground">{findResult.email}</p>
                </div>
                <Badge variant={roleBadgeVariant(findResult.role)} className="capitalize">
                  {findResult.role}
                </Badge>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await setRole({ role: "staff", userId: findResult._id });
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await setRole({ role: "admin", userId: findResult._id });
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Semua akun dengan role staff atau admin.</CardDescription>
        </CardHeader>
        <CardContent>
          {staffList === undefined ? (
            ...
            <p className="text-sm text-muted-foreground">Belum ada staff terdaftar.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {staffList.map((row) => {
                  const isSelf = me !== undefined && me !== null && row._id === me._id;
                  ...
                  return (
                    <TableRow key={row._id}>
                      ...
                      <TableCell className="text-muted-foreground">{formatJoined(row.createdAt)}</TableCell>
                      <TableCell className="text-right">
                        {isSelf ? (
                          <span className="text-xs text-muted-foreground">Tidak dapat mengubah role sendiri</span>
                        ) : (
                              ...
                              <DropdownMenuContent align="end" className="bg-card">
                                <DropdownMenuItem
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      await setRole({ role: "staff", userId: row._id });
                                      toast.success("Role diubah menjadi Staff");
                                    } catch (error) {
                                      toast.error(error instanceof Error ? error.message : "Gagal mengubah role");
                                    } finally {
                                ...
                                <DropdownMenuItem
                                  ...
                                  onClick={async () => {
                                    setRoleChangePendingId(row._id);
                                    try {
                                      await setRole({ role: "admin", userId: row._id });
                                      toast.success("Role diubah menjadi Admin");
                                    } catch (error) {
                                      toast.error(error instanceof Error ? error.message : "Gagal mengubah role");
                                    } finally {
                                      setRoleChangePendingId(null);
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            toast.error(error instanceof Error ? error.message : "Gagal mencabut akses");
          } finally {
