...
export default function ItemCard({ cartQty, item, onAdd, onDecrement, onIncrement }: ItemCardProps) {
  ...
  return (
    ...
    >
      <div className="relative size-20 shrink-0 overflow-hidden rounded-lg bg-muted/40">
        ...
        {soldOut ? (
          <Badge className="absolute inset-x-1 bottom-1 justify-center text-[10px]" variant="destructive">
            Habis
          </Badge>
        ) : null}
      </div>
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          <span className="font-mono text-sm font-medium text-primary">{formatIdr(item.price)}</span>
          {soldOut ? (
            ...
            <div className="flex items-center gap-1 rounded-md border border-border bg-background/80 p-0.5">
              <Button aria-label="Kurangi" size="icon-xs" type="button" variant="ghost" onClick={onDecrement}>
                <MinusIcon className="size-3.5" />
              </Button>
              <span className="min-w-[1.25rem] text-center text-xs font-medium tabular-nums">{cartQty}</span>
              <Button aria-label="Tambah" size="icon-xs" type="button" variant="default" onClick={onIncrement}>
