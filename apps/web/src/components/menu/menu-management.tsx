...
export default function MenuManagement({ mode }: MenuManagementProps) {
  ...
  const [deleteCategoryId, setDeleteCategoryId] = useState<Id<"menuCategories"> | null>(null);
  const [deleteItemId, setDeleteItemId] = useState<Id<"menuItems"> | null>(null);
  ...
  async function handleReorder(categoryId: Id<"menuCategories">, direction: "down" | "up") {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Gagal mengubah urutan";
      toast.error(message);
    }
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-2 space-y-0">
          <div>
            ...
            <CardDescription>Pilih kategori untuk mengelola item.</CardDescription>
          </div>
          {mode === "admin" ? (
            <Button size="sm" type="button" onClick={() => setCategorySheetOpen(true)}>
              <PlusIcon />
              Baru
            </Button>
          ) : null}
          ...
          {loading ? (
            ...
            sortedCategories.map((category) => {
              ...
              return (
                ...
                >
                  <button
                    className={`min-w-0 flex-1 truncate text-left text-sm font-medium ${active ? "text-primary" : ""}`}
                    type="button"
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-3">
          {!selectedCategoryId ? (
            <p className="text-sm text-muted-foreground">Pilih kategori untuk melihat item.</p>
          ) : loading ? (
            <p className="text-sm text-muted-foreground">Memuat…</p>
      ...
      <CategoryFormSheet open={categorySheetOpen} onOpenChange={setCategorySheetOpen} />

      <MenuItemFormSheet
        categories={sortedCategories.map((category) => ({ _id: category._id, name: category.name }))}
