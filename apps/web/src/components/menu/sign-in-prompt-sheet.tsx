...
export default function SignInPromptSheet({ onOpenChange, open, redirectPath }: SignInPromptSheetProps) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-md" side="bottom">
        <SheetHeader>
          ...
          <SheetDescription>Kamu bisa melihat menu tanpa login. Untuk mengirim pesanan ke dapur, masuk dulu.</SheetDescription>
        </SheetHeader>
        <SheetFooter className="flex-col gap-2 sm:flex-col">
          <NavLink className={cn(buttonVariants(), "w-full justify-center")} to={`/sign-in?redirect=${encoded}`}>
            Masuk
          </NavLink>
          <NavLink
