...
export default function CartDrawer({ onRequireAuth, user }: CartDrawerProps) {
  ...
  return (
    <>
      {totalItems > 0 ? (
        <button
          className="fixed bottom-4 left-4 right-4 z-40 flex items-center justify-between gap-3 rounded-full border border-primary/40 bg-primary px-4 py-3 text-primary-foreground shadow-lg transition hover:bg-primary/90 md:left-auto md:right-6 md:max-w-md"
          type="button"
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="flex max-h-[90vh] flex-col" side="bottom">
          ...
          <div className="flex flex-1 flex-col gap-3 overflow-y-auto px-4">
            {lines.map((line) => (
              <div className="flex items-start justify-between gap-3 border-b border-border pb-3" key={line.menuItemId}>
                <div className="min-w-0">
                  <p className="font-medium">{line.name}</p>
                  <p className="font-mono text-xs text-muted-foreground">{formatIdr(line.price)} / item</p>
                </div>
          ...
          <SheetFooter className="flex-col gap-3 border-t border-border pt-4">
              ...
              onClick={() => {
                void (async () => {
                  ...
                  try {
                    ...
                    const message = error instanceof Error ? error.message : "Gagal memesan";
                    toast.error(message);
                  } finally {
                    setSubmitting(false);
                  }
            ...
            >
              {authLoading ? "Memuat…" : authenticated ? "Pesan sekarang" : "Masuk untuk memesan"}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
