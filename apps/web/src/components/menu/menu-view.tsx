...
export default function MenuView({ onRequireAuth }: MenuViewProps) {
  ...
  if (menu === undefined) {
    return <p className="text-sm text-muted-foreground">Memuat menu…</p>;
  }

  if (menu.length === 0) {
    return <p className="text-sm text-muted-foreground">Menu belum tersedia.</p>;
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="-mx-1 flex gap-1 overflow-x-auto pb-1">
        ...
        {categories.map((category) => {
          const active = category._id === effectiveActive && activeCategoryId !== null;
          return (
            <button
              className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${
                active
      ...
      <div className="flex flex-col gap-8">
        {sectionsToShow.map((section) => (
          <section className="flex flex-col gap-3" id={`cat-${section.category._id}`} key={section.category._id}>
            <h3 className="border-l-2 border-primary pl-2 text-sm font-semibold tracking-tight">
