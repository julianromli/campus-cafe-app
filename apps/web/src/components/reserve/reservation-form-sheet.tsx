...
export default function ReservationFormSheet({
  ...
  const createPaymentLink = useAction(api.payments.createReservationPaymentLink);
  const minDate = useMemo(() => getMinDate(), []);
  const maxDate = useMemo(() => getMaxDate(), []);

  const [date, setDate] = useState(isValidDateInput(initialDate) ? initialDate : minDate);
  ...
  const footer = (
    <div className="flex justify-end gap-2">
      ...
      <Button disabled={!table || submitting} onClick={async () => {
        if (!table) {
          return;
        }
    ...
    >
      <div className="flex flex-col gap-5">
        <section className="rounded-md border border-border p-4">
          <div className="text-sm font-medium">{reservationSummary}</div>
          <p className="mt-1 text-sm text-muted-foreground">Only available tables can continue to the payment step.</p>
        </section>
        ...
        <section className="rounded-md border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
          Pembatalan dapat dilakukan minimal 2 jam sebelum jadwal. Refund diproses manual dalam 1-3 hari kerja.
        </section>
      </div>
    </SidePanel>
  );
