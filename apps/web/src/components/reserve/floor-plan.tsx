...
type FloorPlanProps = {
  draggable?: boolean;
  onTableMove?: (table: FloorPlanTable, nextPosition: { positionX: number; positionY: number }) => void;
...
export default function FloorPlan({
  ...
  const [previewPositions, setPreviewPositions] = useState<Record<string, { positionX: number; positionY: number }>>({});
  ...
  const updatePreviewPosition = (table: FloorPlanTable, event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    setPreviewPositions((current) => ({
      ...current,
      [table._id]: clampTablePosition(dragState.startX + deltaX, dragState.startY + deltaY),
    }));
  };
    ...
    <div className="w-full overflow-x-auto rounded-lg border border-border bg-card p-3">
      <svg
        ...
        onPointerMove={(event) => {
          ...
          const table = visibleTables.find((entry) => entry._id === dragState.tableId);
          if (table) {
      ...
      >
        <rect className="fill-background stroke-border" height={FLOOR_PLAN_HEIGHT} width={FLOOR_PLAN_WIDTH} x={0} y={0} />
        {visibleTables.map((table) => {
          ...
          return (
            <g
              ...
              onClick={() => {
                if (!dragState && onTableSelect && (draggable || isSelectable)) {
                  onTableSelect(table);
                }
              }}
              onPointerDown={(event) => {
            ...
            >
              ...
              <rect
                className={cn(statusClasses.fill, statusClasses.border, isSelected ? "stroke-[3]" : "stroke-[2]")}
              ...
              />
              <text
                className="fill-current text-[14px] font-semibold text-foreground"
                textAnchor="middle"
