...
export function clampTablePosition(positionX: number, positionY: number) {
  return {
    positionX: Math.max(0, Math.min(positionX, FLOOR_PLAN_WIDTH - FLOOR_PLAN_TABLE_WIDTH)),
    positionY: Math.max(0, Math.min(positionY, FLOOR_PLAN_HEIGHT - FLOOR_PLAN_TABLE_HEIGHT)),
  };
}
